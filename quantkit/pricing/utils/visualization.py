...
def plot_exercise_boundary_binomial_bsm(
    instrument, # Using the Option dataclass defined earlier
    benchmark_res: Optional['OptionPriceResult'] = None,
    title: str = "American Option Optimal Exercise Boundary"
):
    ...
    if instrument.option_type.value == "put":
        # For a put, you exercise when the stock price falls BELOW the boundary
        ax.fill_between(benchmark_res.boundary_times, 0, benchmark_res.boundary_spots, color="red", alpha=0.1, label="Exercise Zone")
        ax.fill_between(benchmark_res.boundary_times, benchmark_res.boundary_spots, ax.get_ylim()[1] * 1.5, color="green", alpha=0.1, label="Continuation Zone")
    else:
        # For a call, you exercise when the stock price rises ABOVE the boundary
        ax.fill_between(benchmark_res.boundary_spots, benchmark_res.boundary_spots, ax.get_ylim()[1] * 1.5, color="red", alpha=0.1, label="Exercise Zone")
        ax.fill_between(benchmark_res.boundary_times, 0, benchmark_res.boundary_times, color="green", alpha=0.1, label="Continuation Zone")
    ...
    # Invert X-axis so T=0 is today, and T=Maturity is on the right
    # ax.set_xlim(0, benchmark_res.boundary_times[-1])
    
    # # Set reasonable Y-limits based on the data
    # valid_spots = benchmark_res.boundary_spots[~np.isnan(benchmark_res.boundary_spots)]
    # if len(valid_spots) > 0:
    #     y_min = max(0, valid_spots.min() * 0.8)
    #     y_max = valid_spots.max() * 1.2
    #     ax.set_ylim(y_min, y_max)
